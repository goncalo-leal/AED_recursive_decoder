...
static void recursive_decoder(int encoded_idx,int decoded_idx,int good_decoded_size)
{
  ...
  // se (symb_size) > _c_->max_bits então não é possível descodificar mais
  // verificar se a mensagem já acabou
  while (symb_size <= _c_->max_bits && _encoded_message_[initial_encoded_idx + symb_size - 1] != 0){
    
    for (int j = 0 ; j < _c_->n_symbols; j++){
      // length de um símbolo
      int i, h;
      for (i = 0; _c_->data[j].codeword[i]; i++);
      
      // o símbolo _c_->data[j].codeword é do tamanho do símbolo que estamos a considerar?
      if (i == symb_size){
        // as strings são do mesmo tamanho, por isso quando _c_->data[j].codeword[h] acabar já comparamos os dois símbolos
        for (h = 0; _c_->data[j].codeword[h] && _encoded_message_[initial_encoded_idx + h] && _c_->data[j].codeword[h] == _encoded_message_[initial_encoded_idx + h]; h++);

        // se o i e o h forem iguais quer dizer que os símbolos são iguais
        if (i == h){
          _decoded_message_[decoded_pos] = j;
          int idx = initial_encoded_idx + symb_size;
          // se o decoded_idx - good_decoded_size não for zero quer dizer que a mensagem já está errada,
          // se não está veerifica se está certa
          if (decoded_pos - correct_decoded_size == 0 && j == _original_message_[decoded_pos]){
          ...
          // printf("I'm back bitch\n");
          // printf("endoded_idx = %d; decoded_idx = %d; good_decoded_size = %d\n", initial_encoded_idx + symb_size, decoded_pos, correct_decoded_size);
        }
  ...
  // quando sai do while ou não consegue descodificar mais nada ou já correu todas as possibilidades
  return;
}
